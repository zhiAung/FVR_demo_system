  ...
  <div class="main">
    <div class="part1">
      <div class="upload_image">
          <h3><font color="white">请上传图片</font></h3>
          <div class="upload">
      ...
      <!--输入表单-->
      <div class="button">
        <button v-on:click="Identify" type="submit">识别</button>
      </div>
      <div class="error">
        <p><font color="white">{{error}}</font></p>
      </div>
      <div class="result" v-if = "resultname === '0'">
      </div>
      <div class="result1" v-else-if = "resultname === '-1'">
        <p><font color="white">不存在该用户</font></p>
      </div>
      <div class="result2" v-else>
        <p><font color="white">识别到：{{resultname}}用户</font></p>
      </div>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      Identify: function() {
       if (this.imgList.length == 0) {
          ...
          let url = 'http://10.108.246.53:5000/api/identification'
          let headers = {
            'Content-Type': 'multipart/form-data'
          }
          axios.post(url, formData, {headers: headers})
          .then(response => {
            this.resultname = response.data.resultNumber
          })
          .catch(error => {
            console.log(error)
          })
...
<style scoped>
  ...
  .main {
    ...
    background-image: url('../assets/0351114624.jpg');
